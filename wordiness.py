...
class Sentence:
    ...
    def find( self, match ):
        pos = self.text.lower().find( match )
        length = len(match)
        if pos + length < len(self.text):
            if self.text[pos+length] == ',':
                length += 1 # include comma
                # wait, if we included a comma at the end, there may be a
                # comma at the beginning and we should eliminate the entire
                # paranthetical phrase.
                if pos >= 2 and self.text[pos-2:pos] == ', ':
                    pos -= 2
                    length += 2
            elif self.text[pos + length] != ' ' and \
                self.text[pos+length] != '.':
                return (-1,0)
            if pos > 1 and self.text[pos-1] != ' ':
                # should loop and try match again.
                return (-1,0)
        return (pos,length)

    def splice( self, pos, length, replacement ):
        self.text = self.text[:pos].strip() + " " + replacement + " " + self.text[pos+length:].strip()
        if pos == 0:
            self.fixSentenceStart()

    def fixSentenceStart( self ):
...
def MakeSentences(text):
    ...
    for ch in text:
        if ch == "." or ch == "?":
            result.append( Sentence( part + ch ) )
            part = ""
        else:
            part += ch
...
class Rule:
    ...
    def process(self, sentence):
        (pos,length) = sentence.find( self.match )
        if pos >= 0:
            sentence.splice( pos, length, self.replace )
            return True
        else:
            return False
...
class Analyzer:
    ...
    def readRules(self):
        for line in RuleText.split("\n"):
            ...
            self.addRule(line )
        self.rules.sort( lambda x,y: len(y.match)-len(x.match))

    def addRule(self, ruleLine):
        a = ruleLine.split(" -> ")
        if len(a) == 1:
            self.rules.append( Rule( a[0], "" ))
        else:
            self.rules.append( Rule( a[0], a[1] ))
...
for paragraph in text.split("\n"):
    for sentence in MakeSentences( paragraph ):
        an.process( sentence )
        a = sentence.otext + " "
        b = sentence.text + " "
        for blk in difflib.SequenceMatcher(None, a, b).get_opcodes():
            if blk[0] == 'replace':
                out.append( 0 ) 
                out.append( a[blk[1]:blk[2]] )
                out.append( b[blk[3]:blk[4]] )
            elif blk[0] == 'delete':
                out.append( 1 ) 
                out.append( a[blk[1]:blk[2]] )
            elif blk[0] == 'insert':
